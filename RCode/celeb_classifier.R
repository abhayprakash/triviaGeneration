...
cross_validate_SVM_PRFA <- function(container, nfold, method = "C-classification", cross = 0, cost = 100, tolerance = 0.001, kernel = "radial")
{
  ...
  for (i in sort(unique(rand))) {
    model <- svm(x = alldata[rand != i, ], y = allcodes[rand != i], gamma= 0.001, method = method, cross = cross, cost = cost, tolerance = tolerance, kernel = kernel)
...
model <- cross_validate_SVM_PRFA(container, 5, cost = 17, tolerance = 0.21, kernel = "linear")
...
model <- train_model(container, algorithm=c("SVM"), method = "C-classification", cross = 5, cost = 100, tolerance = 0.21, kernel = "linear")
