...
cross_validate_SVM_PRFA <- function(container, nfold, method = "C-classification", cross = 0, cost = 100, kernel = "radial")
{
  ...
  for (i in sort(unique(rand))) {
    ...
    true_labels <- floor((as.numeric(allcodes[rand == i]) - 1)/2.0)
    predicted_labels <- floor((as.numeric(pred, mode = class(true_labels)) - 1)/2.0)
...
#write.table(results,"abhi_predicted_classify_1_0_rich_IMb_heuristic.txt", sep='\t',row.names=F)

# getting performance
true_labels <- as.vector(results$CLASS)
predicted_labels <- floor((as.vector(results$SVM_LABEL, mode = class(true_labels)) - 1)/2.0)
